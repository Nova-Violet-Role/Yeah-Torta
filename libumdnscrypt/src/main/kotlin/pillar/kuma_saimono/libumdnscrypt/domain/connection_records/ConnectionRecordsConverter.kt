...
import pillar.kuma_saimono.libumdnscrypt.vpn.service.VpnBuilder
import java.io.IOException
import java.util.concurrent.ArrayBlockingQueue
import javax.inject.Inject

private const val REVERSE_LOOKUP_QUEUE_CAPACITY = 32
private const val IP_TO_HOST_ADDRESS_MAP_SIZE = ServiceVPN.LINES_IN_DNS_QUERY_RAW_RECORDS
...
class ConnectionRecordsConverter @Inject constructor(
    ...
    // The connection-records block status reads the datapath verdict carried on
    // PacketRecord.allowed (the native engine's per-connection verdict, Warden-bound at the
    // VPN datapath). The legacy ROOT-mode iptables allow-set mirror was removed together with
    // the Garmatin firewall; the Warden is the sovereign firewall now.
    private fun isUidBlocked(packetRecord: PacketRecord) =
        firewallEnabled && !packetRecord.allowed
